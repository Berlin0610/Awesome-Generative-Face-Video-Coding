...
class GeneratorFullModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.loss_weights['headpose'] != 0:
            transform_hopenet =  transforms.Compose([#transforms.ToPILImage(),
            ...
            #driving_224 = transform_hopenet(Image.fromarray(np.uint8(np.transpose(x['driving'].data.cpu().numpy(), [0, 2, 3, 1])[0]))) ##########
            
            #driving_224 = transform_hopenet(x['driving'].cpu().numpy().squeeze(0)).unsqueeze(0).cuda()
